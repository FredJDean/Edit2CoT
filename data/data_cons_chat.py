...
INSTRUCT_TEXT = (
    ...
    "question based on the extracted edit fact, in this process you need to decompose the question and extract the "
    "chain of thought based on the edit fact, put the extraction of the edit fact and the steps to decompose the "
    "question in <think></think> tags and put the answer in the <answer></answer> tags."
)
# 读取hotpot中的实体
    ...
    with open('generated_1000_with_instruct.jsonl', 'a', encoding='utf-8') as f:
        for i in tqdm(range(num_to_generate)):
            ...
            try:
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system", "content": "You are a helpful assistant generating diverse and high-quality instruction-following data.\n"
                                                      ...
                                                      "2. Use creative but plausible edit contexts and questions.\n"
                                                      "3. Can't have any extraneous strings other than json.\n"
                                                      "4. The format of the example data must be followed.\n"
                                                      f"5. Be sure to generate content about entity {entities[i]}.\n"
                                                      "6. The keys in json format must be Input, Output\n"
                                                      "7. Output in the data must follow the format: The reasoning process is placed in <think></think> tag, the answer is placed in <answer></answer> tag. No other characters are allowed.\n"},
                ...
                #     messages=[
                #         {"role": "system", "content":
                #          "You are a data generation assistant. Your goal is to generate creative and diverse instruction-style training data.\n"
                #          "1. Vary entity types, domains, and reasoning styles.\n"
                #          "2. Use only JSON format.\n"
                #          "3. No duplication or stylistic imitation of examples.\n"
                #          "4. Use <think></think> and <answer></answer> in the output.\n"
                #          "5. Be creative, realistic, and diverse."},
                #         {"role": "user", "content":
                #          f"""Based on the following examples, generate a new sample with a different structure, domain, or entity type:\n\n{example_prompt}"""}
                #     ],
                #     temperature=0.8,
                #     top_p=0.95,
                #     max_tokens=500,
...
# 主流程
def main():
    example_path = "unstructure_reason.jsonl"
    ...
    generate_data(examples, num_to_generate=3000)
